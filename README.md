...
6. 完成后下载 16 位 TIFF。右侧对比图按当前亮度范围各显示最亮 3 颗和最暗 3 颗，每颗左右并排标出名称与亮度。覆盖图显示图像检测点、星表匹配/补回点、没有图像点源证据的推算位置，以及 WCS 解算或相机模型投影区域。
...
真实星表模式使用随包 ASTAP 命令行程序与 D05/G05/W08 索引。RAW/TIFF/JPG 的全分辨率 16 位灰度副本只用于解算；广角图像使用多个重叠视场分别拟合畸变，优先采用离图块边缘较远的 WCS。星点经 SEP 检出后与 W08 交叉匹配；程序还会把局部 WCS 映射回 W08 星表位置，并在预期位置附近以较低门限重新检查紧致点源，因此可以补回部分 SEP 主检测漏掉的亮星。覆盖图分别计数图像检测点、星表推算位置以及附近通过局部 SEP 点源检查的补回点，并绘出当前已有 WCS 或可投影相机模型的区域。星表位置若没有局部点源图像证据，只作为未确认候选显示，不参与柔焦；因此 `图像确认 / 推算候选` 是当前 ΔG 范围内的可见 W08 目录核对结果，不代表目录外或更暗星点的完备率。ASTAP 无法确认时，Seiza 先以 Gaia DR3 G≤15 目录做本机盲解，再用 W08 星表做独立复核。ASTAP 和轻量 Seiza 仍未解出时，会按需下载 Gaia DR3 G≤17 深度目录（约额外 1.54 GB）再次分区解算。首次 Seiza 备用数据约 2 GB；处理照片与星点坐标不会上传。W08 约完整至 G=8 等、精度 0.1 等，没有 Gaia source_id 或 BP-RP；光晕颜色按图像内逐星 RGB 取样。星点比较图的常用名称与 Bayer/Flamsteed 标识参考 HYG v4.1；没有名称匹配时用 J2000 坐标标识，柔焦亮度仍只用 W08 的 G 星等。[HYG v4.1 数据与许可](https://github.com/astronexus/HYG-Database/blob/main/hyg/README.md)。星表模式在相机焦距/画幅信息不足或板解算失败时会提示失败；可切换到图像亮度模式继续处理。相机 EXIF 不提供 35 mm 等效焦距时，只能依据程序内已识别的相机型号估算画幅。

对于至少 120 个分布式 W08 匹配且 P90≤6 solver px 的全幅相机模型，回搜范围会根据实测残差放宽（上限 600 角秒），并只在局部 SEP 找到紧致图像星像时确认和柔焦；未通过复核的位置保留为仅推算。这样的高可信模型不再重复耗时的 ASTAP 全图块盲搜。广角边角如果仍缺图像证据，会直接留在覆盖图中显示。
...
`LZQ_6627.NEF` 的 1.4.18 本机复核使用 W08 的 ΔG≤5 范围，SEP 检出 3,207 个图像点源候选；扩大回搜范围后，W08/Gaia 匹配得到 516 个有图像证据的星表位置，其中 152 颗进入默认柔焦范围。同范围内另有 819 个星表位置没有找到紧致图像点源证据，只在覆盖图显示为“仅推算”，不参与柔焦。坐标模型通过 300 个分布式 W08 匹配复核，P90 为 5.15 solver px；几何解算成功不等于全部星点已识别，覆盖图按格显示实测/推算数量，并单独标出坐标模型范围与地景掩码外的空白区域。
