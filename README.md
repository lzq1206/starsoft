...
6. 完成后下载 16 位 TIFF。右侧对比图按当前亮度范围各显示最亮 3 颗和最暗 3 颗，每颗左右并排标出名称与亮度。覆盖图分别显示图像确认点、WCS 引导补回点和未确认推算点；逐星对比会标注图像确认或 WCS 引导补回。没有局部星像证据的推算点不会柔焦。
...
真实星表模式使用随包 ASTAP 命令行程序与 D05/G05/W08 索引。RAW/TIFF/JPG 的全分辨率 16 位灰度副本只用于解算；广角图像使用多个重叠视场分别拟合畸变，优先采用离图块边缘较远的 WCS。星点经 SEP 检出后与 W08 交叉匹配；程序还会把局部 WCS 映射回 W08 星表，在预期位置附近以较低门限重新检查紧致点源，补回部分 SEP 主检测漏掉的亮星。对于经分布式星表匹配验证的广角相机模型，当前 ΔG 范围内其余落在天空掩码内的 W08 投影位置会引导局部回搜，半径最高 18 个检测像素。只有回搜到紧致 SEP 图像星像，才用实际图像中心和星表 G 星等加入柔焦；没有局部星像证据的推算位置只留在覆盖图，不柔焦。覆盖图分别统计图像确认点、WCS 引导补回点和未确认推算点，不宣称星点完整。ASTAP 无法确认时，Seiza 先以 Gaia DR3 G≤15 目录做本机盲解，再用 W08 星表做独立复核。ASTAP 和轻量 Seiza 仍未解出时，会按需下载 Gaia DR3 G≤17 深度目录（约额外 1.54 GB）再次分区解算。首次 Seiza 备用数据约 2 GB；处理照片与星点坐标不会上传。W08 约完整至 G=8 等、精度 0.1 等，没有 Gaia source_id 或 BP-RP；光晕颜色按图像内逐星 RGB 取样。星点比较图的常用名称与 Bayer/Flamsteed 标识参考 HYG v4.1；没有名称匹配时用 J2000 坐标标识，柔焦亮度仍只用 W08 的 G 星等。[HYG v4.1 数据与许可](https://github.com/astronexus/HYG-Database/blob/main/hyg/README.md)。星表模式在相机焦距/画幅信息不足或板解算失败时会提示失败；可切换到图像亮度模式继续处理。相机 EXIF 不提供 35 mm 等效焦距时，只能依据程序内已识别的相机型号估算画幅。

对于至少 120 个分布式 W08 匹配且 P90≤6 solver px 的全幅相机模型，回搜范围会根据实测残差放宽（上限 600 角秒），局部搜索半径最多 18 个检测像素。找到紧致 SEP 图像星像后才按实测中心柔焦；其余 W08 投影候选只显示为未确认，不柔焦。覆盖图按格显示图像确认与未确认数量，这样能直观看到广角边角覆盖，又不会把推算位置冒充成完整识别。这样的高可信模型不再重复耗时的 ASTAP 全图块盲搜。
...
`LZQ_6627.NEF` 的本机复核使用默认 W08 ΔG≤5，SEP 检出 3,207 个图像点源候选；本次选出 152 个柔焦星点，其中 135 个由 WCS 位置引导回搜后找到局部图像星像并补回，另 17 个为直接匹配。当前范围内另有 438 个仅推算位置没有紧致星像证据，只显示在覆盖图、不参与柔焦。相机模型通过 300 个分布式 W08 匹配复核，P90 为 5.15 solver px；最亮和最暗样例的前后对比已检查，柔光保持正圆，未确认位置与地景不会被标作已识别星点。
